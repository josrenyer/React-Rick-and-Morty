...
export default function CardDetail(){
	...
	const tamanoNumero=numero.toString().length;
  ...
  async function getApi(url){
	...
	if(numero !== NaN && numero > 0 && numero <827 && tamanoNumero === id.length){
  	return(<>
		...
		return(<>
			<Nav/>
			<div className="CardDetalle">
				<h1>Por favor ingrese un id mayor de 0 y menor de 827, no se permiten letras</h1>
			</div>
			<Link to="/Home"><button className="botonDetalle">Atras</button></Link>
		</>
		)
